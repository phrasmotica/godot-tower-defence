...
func handle_enemy_hit(enemy: Enemy) -> void:
	if _bullet.projectile_stats.penetration_count > 0:
		...
		Logger.debug("Collided with %s, %d penetration(s) remaining" % [enemy.name, _bullet.projectile_stats.penetration_count])
		return

	Logger.debug("Freeing after collision with %s" % enemy.name)
	_bullet.queue_free()
