...
func handle_enemy_hit(enemy: Enemy) -> void:
	if _ricochets.try_deduct():
		Logger.info("Collided with %s, %d ricochet(s) remaining" % [enemy.name, _ricochets.count()])
		ricochet(enemy)
		return

	Logger.info("Freeing after collision with %s, no ricochets remaining" % enemy.name)
	_ricochet_bullet.queue_free()
...
func ricochet(enemy: Enemy) -> void:
	...
	if nearest_enemy:
		Logger.info("Ricocheting towards %s" % nearest_enemy.name)
		...
		Logger.info("No nearby enemy to ricochet towards!")
